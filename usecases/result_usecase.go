...
func (uc *ResultUsecase) GetResultValueById(ctx context.Context, key string) (string, error) {
	...
	if err != nil {
		...
		result, err := uc.resultRepository.GetResultById(key)

		if err != nil {
			return "", errors.New("no value for provided key")
		}

		uc.cache.Set(ctx, key, result.Value, time.Hour)
